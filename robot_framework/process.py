...
from robot_framework.subprocesses.process.patient.update_patient_info import update_patient_info
from robot_framework.subprocesses.process.patient.create_event import create_event_if_not_created
from robot_framework.subprocesses.process.patient.create_booking_reminders import create_booking_reminders
from robot_framework.subprocesses.process.document.handle_discharge_document import handle_discharge_document
from robot_framework.subprocesses.process.document.send_discharge_document import check_and_send_discharge_document
from robot_framework.subprocesses.process.document.create_medical_record import check_and_create_medical_record_document
from robot_framework.subprocesses.process.romexis.romexis_images_handler import get_images_from_romexis
from robot_framework.subprocesses.process.edi.edi_portal_handler import EdiContext
from robot_framework.subprocesses.process.edi.edi_portal_handler import edi_portal_handler
from robot_framework.subprocesses.process.edi.get_files_for_edi_portal import prepare_edi_portal_documents
...
def process(
    ...
    try:
        ...
        template_name, discharge_document_filename = update_patient_info(
        ...
        administrative_note = (
            ...
            if orchestrator_connection.administrative_note and len(orchestrator_connection.administrative_note) > 0
            else None
        )
        ...
        try:
            ...
            receipt_pdf = edi_portal_handler(ctx)
        ...
        orchestrator_connection.log_trace(f"Found {len(list_of_documents)} existing EDI Portal document.")

        if not list_of_documents:
            orchestrator_connection.log_trace("No existing EDI Portal document found, creating a new one.")
        ...
        orchestrator_connection.log_trace(
            "Checking if administrative note exists."
        )
        ...
        if not result:
            ...
            orchestrator_connection.log_info("Administrative note created successfully.")
